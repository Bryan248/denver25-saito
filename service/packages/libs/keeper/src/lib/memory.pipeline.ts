...
import { MemoryAuthService } from "@saito/keeper";
...
export class DefaultMemoryKeeperPipeline implements MemoryKeeperPipeline {
   ...
   */
  async authorizeMemory(memoryId: string, requesterId: string) :Promise<AuthorizeMemoryResult> {
    const signature = await this.memoryAuthService.constructGrantMemoryEIP712Signature(memoryId, requesterId);
    return await this.memoryAuthService.grantMemoryAccess(memoryId, requesterId, signature);
  }
